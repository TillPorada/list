<template>
  <div>
    <v-expansion-panels class="mb-6">
      <v-expansion-panel v-for="amn in amnList" :key="amn.id">
        <v-expansion-panel-title disable-icon-rotate>
          ...
          <template v-slot:actions>
            <v-icon color="warning" icon="mdi-arrow-down-circle" style="margin-right: 10px;"> </v-icon>
          </template>
        ...
        <v-expansion-panel-text>
          ...
          <div style="margin-top: 20px;">
            ...
            <v-icon color="warning" icon="mdi-plus" @click="addAMN"></v-icon>
          </div>
        </v-expansion-panel-text>
      </v-expansion-panel>
    </v-expansion-panels>
...
const fetchamnList = async () => {
  try {
    ...
    amnList.value = response.data.results.map(
      (amn: any, index: number) => ({
        id: index + 1,
        name: amn.name,
        url: amn.url,
